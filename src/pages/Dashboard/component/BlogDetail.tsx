        ...
        <Layout>
          ...
          <Content>
            {loading ? (
              ...
              blogDetail && (
                <Row gutter={[24, 16]} className='blog-detail-content' style={{ lineHeight: '1.6', marginBottom: '20px' }}>
                  <Col xs={24} xl={18}>
                    <div style={{ margin: '0 10px' }}>
                      <div style={{ fontSize: 'larger', fontStyle: 'italic', marginBottom: '10px' }}> {blogDetail.description}</div>
                      <div dangerouslySetInnerHTML={{ __html: blogDetail.content.replace('<img>', `<img style='display: block; margin: 0 auto;width: 80%' src='${blogDetail.image}' alt='${blogDetail.title}'>`) }} style={{ fontFamily: 'sans-serif', fontSize: 'large' }} />
                      <div className='pt-5'>
                        <p style={{ marginBottom: '10px' }}>
                          Tác giả: {blogDetail.account.name} - {new Date(blogDetail.timeCreate).toLocaleDateString('vi-VN')}
                          <br />
                          Danh mục: <Link to={`../${blogDetail.tag.name}`} className='no-underline'>{blogDetail.tag.name}</Link>
                        </p>
                      </div>
                    </div>
                  </Col>
                    ...
                    {blogByAccount.slice(0, 4).map((blogByAccounts) => (
                      <Col key={blogByAccounts.id} xs={24} sm={12} md={16} lg={12} xl={6}>
                        <Link to={`/blog/${blogByAccounts.id}`} style={{ textDecoration: 'none' }}>
                          <Card hoverable style={{ width: '100%' }} cover={<img alt='example' src={blogByAccounts.avatar} />}>
                            <Meta title={blogByAccounts.title} description={truncateText(blogByAccounts.description, 20)} />
                          </Card>
                        </Link>
                      </Col>
                    ))}
