  ...
  const onSave = async () => {
    try {
      ...
      if (password !== newPassword) {
        ...
        const response: any = await HttpService.put(END_POINT_API.Users.changePassword(), payload)
        if (response && response?.id) {
          notification.success({message: t('changePassword.changePasswordSuccessfully') })
          await handClose()
          handleLogout()
          setServerError({
            status: 0,
