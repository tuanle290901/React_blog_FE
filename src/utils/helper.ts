...
export const hasPermission = (allowedRoles: ROLE[], groupProfiles?: any[]) => {
  let isHasPermission = false
  if (allowedRoles.length === 0) {
    isHasPermission = true
  }
  console.log(groupProfiles, 'alow role')
  for (const role of allowedRoles) {
    const foundRole = groupProfiles?.find((group) => group.authority === role)
    if (foundRole) {
      isHasPermission = true
      break
    }
